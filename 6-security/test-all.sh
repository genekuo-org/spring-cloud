...
ACCESS_TOKEN=$(curl -k https://writer:secret@$HOST:$PORT/oauth/token -d grant_type=password -d username=demo -d password=password -s | jq .access_token -r)
# For OIDC
# ACCESS_TOKEN=$(curl --request POST \
#  --url 'https://{tenant-domain}.auth0.com/oauth/token' \
#  --header 'content-type: application/json' \
#  --data '{"grant_type":"password", "username":"...", "password":"...", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read product:write", "client_id":"...", "client_secret":"..."}' -s | jq -r .access_token)
...
READER_ACCESS_TOKEN=$(curl -k https://reader:secret@$HOST:$PORT/oauth/token -d grant_type=password -d username=demo -d password=password -s | jq .access_token -r)
# For OIDC
# READER_ACCESS_TOKEN=$(curl --request POST \
#  --url 'https://{tenant-domain}.auth0.com/oauth/token' \
#  --header 'content-type: application/json' \
#  --data '{"grant_type":"password", "username":"...", "password":"...", "audience":"https://localhost:8443/product-composite", "scope":"openid email product:read", "client_id":"...", "client_secret":"..."}' -s | jq -r .access_token)
